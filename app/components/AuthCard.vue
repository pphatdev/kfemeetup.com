<template>
  <div class="w-full max-w-94.25 min-h-147 bg-background text-foreground rounded-2xl shadow-2xl shadow-primary/10 p-6 sm:p-9 flex flex-col justify-between transition-all duration-300">
    <!-- Header & Segmented Tab Switcher -->
    <div>
      <div class="w-full max-w-76 h-10.25 bg-[#F8F8F8] p-1 rounded-[15px] flex items-center mx-auto mb-6">
        <button 
          type="button"
          @click="activeTab = 'login'"
          :class="[
            'flex-1 h-7.75 text-xs font-bold rounded-[10px] transition-all duration-200 flex items-center justify-center',
            activeTab === 'login' 
      ...
      <!-- Login Form -->
      <form v-if="activeTab === 'login'" @submit.prevent="handleSubmit" class="w-full max-w-76 mx-auto flex flex-col gap-4">
        ...
        <div class="flex flex-col gap-1">
          ...
          <div class="relative">
            <input 
              ...
              required
              class="w-full h-8.25 px-3.5 text-xs text-foreground bg-background border border-[#C7C7CC] rounded-[9.5px] focus:outline-none focus:border-primary-accent focus:ring-2 focus:ring-primary-accent/20 transition-all placeholder:text-neutral-400"
            />
          </div>
        </div>
        ...
        <button 
          type="submit"
          class="w-full h-8.75 mt-1 bg-primary-linear hover:opacity-95 text-primary-foreground font-bold text-xs rounded-[10px] shadow-sm active:scale-[0.99] transition-all flex items-center justify-center"
        >
          Login
        </button>
      </form>
      ...
      <!-- Register Form -->
      <form v-else @submit.prevent="handleSubmit" class="w-full max-w-76 mx-auto flex flex-col gap-3.5">
        <!-- Full Name -->
        <div class="flex flex-col gap-1">
      ...
      <div class="w-full max-w-76 mx-auto my-4 flex items-center gap-3">
      ...
      <div class="w-full max-w-76 mx-auto flex flex-col gap-2.5">
        <!-- Google Button -->
        <button 
          type="button"
          @click="handleSocial('Google')"
          class="w-full h-7.75 bg-white border border-[#E5E5E5] rounded-[7.5px] text-xs font-semibold text-neutral-700 hover:bg-neutral-50 transition-all flex items-center justify-center gap-2 shadow-2xs"
        >
